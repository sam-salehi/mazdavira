...
export function Sidebar({selectedPaper,onSelectPaper,onClose,chosenPapers,setChosenPapers}:{selectedPaper:string,onSelectPaper:(s:string)=>void,onClose:()=>void,chosenPapers:chosenPaper[],setChosenPapers:(papers:chosenPaper[])=>void}) {
    ...
    const {addSummaryPrompt,updateSummaryPrompt,addQuestionPrompt,updateQuestionPrompt,removePrompt} = useChatContext();

    const generateSummary = async function(arxiv: string,pdfLink:string, callback:()=>void,errorCallback:()=>void) { //FIXME: move api requests? use contex?
    ...
    return <div className="bg-black fixed border-l border-gray-500 right-0 top-0 h-full w-1/4 p-7">
        ...
        <div className="mt-8 grid grid-cols-1 gap-4">
            {chosenPapers.map(paper=> <PaperCard key={paper.arxiv} id={paper.arxiv} title={paper.title} authors={paper.authors} summary={paper.summary} year={paper.year} link={paper.link} selected={selectedPaper === paper.arxiv} onSummaryGeneration={generateSummary} onQuestionResponseGeneration={generateQuestionResponse} onClick={() => onSelectPaper(paper.arxiv)} onClose={() => setChosenPapers(chosenPapers.length > 1? chosenPapers.filter((p) => p.arxiv !== paper.arxiv):[])}/>)}
        </div>
